...
class GridTest {
    ...
    void candidateIndicesByIndex_1x1WithNoClues() {
        var grid = Sudoku_1x1.UNSOLVED.grid();
        var exp = List.of(new GridIndexCandidateIndices(0, List.of(0)));
        var act = grid.candidatesPerIndex();
        assertEquals(exp, act);
    }
    ...
    void candidateIndicesByIndex_2x2_unsolved3Unknowns() {
        var grid = Sudoku_2x2.UNSOLVED_3_UNKNOWN.grid();
        var exp = List.of(
                new GridIndexCandidateIndices(0, List.of(0)),
                new GridIndexCandidateIndices(6, List.of(0)),
                new GridIndexCandidateIndices(8, List.of(2)));
        var act = grid.candidatesPerIndex();
        assertEquals(exp, act);
    }
}
